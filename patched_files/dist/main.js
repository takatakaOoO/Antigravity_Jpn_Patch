...
electron_1.app
    ...
    if (!fs.existsSync(languageServer_1.LS_BINARY)) {
        const msg = `言語サーバーのバイナリが見つかりません：\n${languageServer_1.LS_BINARY}\n\n有効なパスを設定してください。`;
        if (HEADLESS) {
            console.error('ERROR:', msg);
        }
        else {
            await electron_1.dialog.showErrorBox('バイナリが見つかりません', msg);
        }
        electron_1.app.quit();
        return;
    }
    ...
    try {
        ...
        if (HEADLESS) {
            ...
            await electron_1.dialog.showErrorBox('起動に失敗しました', msg);
        }
        electron_1.app.quit();
        return;
    }
    ...
    if (!HEADLESS) {
        ...
        (0, tray_1.createTray)([
            {
                id: 'running-agents',
                label: 'エージェントは実行されていません',
                enabled: false,
            },
            { type: 'separator' },
            {
                label: `${electron_1.app.getName()} を開く`,
...
electron_1.app.on('before-quit', async (event) => {
    ...
    const options = {
        ...
        title: '終了の確認',
        message: '本当に終了しますか？',
        detail: 'エージェントやバックグラウンドタスクが実行中の可能性があります。',
    };
