...
function startLanguageServer(port, csrf, headless) {
    return new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
            if (!resolved) {
                resolved = true;
                reject(new Error(`タイムアウト：言語サーバーが${LS_STARTUP_TIMEOUT_MS / 1000}秒以内にポートを報告しませんでした`));
            }
        ...
        rl.on('line', (line) => {
            ...
            if (authMatch) {
                console.log('\n' + '='.repeat(60));
                console.log('  認証のために以下のURLにアクセスしてください。');
                console.log('  認証後、認証コードを以下に貼り付けてください。');
        ...
        const exitPromise = new Promise((exitResolve) => {
            _lsProcess.on('exit', (code, signal) => {
                ...
                if (!resolved) {
                    resolved = true;
                    clearTimeout(timer);
                    reject(new Error(`言語サーバーが予期せず終了しました (code=${code}, signal=${signal})`));
                }
...
function monitorLsCrashInternal(handle, port, csrf, options) {
    void handle.exitPromise.then(async (exitInfo) => {
        ...
        if (_restartCount >= MAX_RESTARTS) {
            const msg = `言語サーバーが${MAX_RESTARTS}回連続でクラッシュしました。再起動を中止します。`;
            console.error(msg);
            return;
        }
        _restartCount++;
