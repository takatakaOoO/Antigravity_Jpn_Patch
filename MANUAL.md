...
3. **反映させる**
   ファイルを保存した後、同じフォルダにある **`install.bat`** をダブルクリックして実行します。
   ※実行中に「現在のクリーンな状態でバックアップファイルを上書きしますか？」と聞かれた場合は、**`n`** を入力してEnterキーを押してください（元の安全なバックアップを保持するため）。
...
2. このパッチフォルダにある **`install.bat`** をダブルクリックして実行します。
3. 実行中に「現在のクリーンな状態でバックアップファイルを上書きしますか？」と聞かれた場合は、**`y`** を入力してEnterキーを押してください（アップデートされた最新の純正版を新しいバックアップとして保存するため）。
