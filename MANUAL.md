...
3. **反映させる**
   ...
   ※実行中に「バックアップを上書き作成しますか？」と聞かれた場合は、**`n`** を入力してEnterキーを押してください（元の安全なバックアップを保持するため）。
...
2. このパッチフォルダにある **`install.ps1`** を再度右クリックして「PowerShell で実行」します。
3. 実行中に「バックアップを上書き作成しますか？」と聞かれた場合は、**`y`** を入力してEnterキーを押してください（アップデートされた最新の純正版を新しいバックアップとして保存するため）。
